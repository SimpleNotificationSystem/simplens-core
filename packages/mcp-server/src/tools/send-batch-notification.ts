...
export function registerSendBatchNotification(server: McpServer, getCredentials: () => UserCredentials) {
    server.tool(
        ...
        'Send the same notification to multiple recipients at once. Use list_plugins first to discover available channels and their required fields.',
        {
            ...
            channel: z.array(z.string()).describe('Array of channels to send via'),
            ...
            recipients: z.array(z.object({
                ...
                variables: z.record(z.string(), z.string()).optional().describe('Per-recipient template variables'),
            }).passthrough()).describe('Array of recipients with their details'),
        ...
        async (params) => {
            try {
                ...
                return {
                    content: [{
                        type: 'text' as const,
                        text: `Failed to send batch notification: ${error instanceof Error ? error.message : String(error)}`,
                    }],
                    isError: true,
                };
            }
